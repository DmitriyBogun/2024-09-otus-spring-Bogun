...
class ModelsCommonTest {
    ...
    void shouldBeNoOneToOneRelationshipsInModelClasses(Class<?> entityClass) {
        ...
        boolean oneToOneRelationshipExists = Arrays.stream(entityClass.getDeclaredFields())
    ...
    void shouldMappedForBidirectionalRelationshipsInModelClasses(Class<?> entityClass) {
        ...
        var hasBidirectionalRelationshipsWithoutMappedBy = relationsEntries.entrySet().stream()
                .anyMatch(relationEntry -> {
                    Map<? extends Class<?>, Field> reverseRelations = findAllRelationsEntry(relationEntry.getKey());
                    Field reverseRelationField = reverseRelations.get(entityClass);
                    if (isNull(reverseRelationField)) {
                        return false;
                    }
                    String relationFieldName = relationEntry.getValue().getName();
                    String reverseRelationFieldName = reverseRelationField.getName();
                    String mappedByValue = getRelationAnnotationArgumentValue(relationEntry.getValue(),
                            "mappedBy", String.class);
                    String reverseMappedByValue = getRelationAnnotationArgumentValue(reverseRelationField,
                            "mappedBy", String.class);
